            ...
            <ul :virtual-render="true" id="questionUl">
                ...
                <div v-for="(item,index) in comName" :key="index">
                    ...
                    <component
                        :is="item.name"
                        :title="index + 1"
                        :answerdata="getquestion"
                        @confirms="addQuestionData"
                        @add="openAddDialoag(index)"
                        @func="openDelDialoag(index)">
                    </component>
                    <!-- @click="handleClick"
                    <bk-button theme="default" @click="handleClick" class="mr10 questionType" type="submit" size="small" title="搜索题目">题库导入
                    </bk-button> -->
                </div>
            </ul>
...
<script>
    ...
    export default {
        ...
        created () {
            this.course_id = this.$store.state.currentCourseId
            this.getquestion = this.$route.query.param
